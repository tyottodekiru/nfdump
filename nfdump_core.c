...
// Global variables
static struct nfdump_btf_bpf *skel = NULL;
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
    ...
    // Determine path type
    const char *path_type = "SLOW";
    if (strncmp(info->table_name, "route_fast", 10) == 0 ||
        strncmp(info->table_name, "local_deliv", 11) == 0 ||
        strncmp(info->table_name, "fwd_fast", 8) == 0 ||
        strncmp(info->chain_name, "FASTPATH", 8) == 0) {
        path_type = "FAST";
    }
...
// Argp configuration
static char doc[] = "NetFilter CO-RE Monitor - Track packets through netfilter tables and chains using BPF CO-RE";
...
int main(int argc, char **argv) {
    ...
    // Attach BPF programs
    err = nfdump_btf_bpf__attach(skel);
    if (err) {
    ...
    if (cfg.verbose) {
        ...
        printf("Monitoring netfilter hooks using fentry probes...\n");
    }
    ...
    printf("Time              PF       Table       Chain       Src IP          Dst IP          Proto  SPort  DPort   Len  Path\n");
    printf("----------------------------------------------------------------------------------------------------------------------\n");
